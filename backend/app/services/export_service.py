"""
文档导出服务 —— 从 Markdown 生成 DOCX 和 PDF。

从 api/export.py 的业务逻辑抽离，解耦 FastAPI。
"""
...
def _build_pdf(blocks: list[dict]) -> BytesIO:
    ...
    styles.add(ParagraphStyle(
        ...
        borderPadding=8, backColor=HexColor('#FCF8F2'),
    ...
    for block in blocks:
        ...
        if t == 'h1':
            ...
            if table_data:
                ...
                tbl = Table(table_data, colWidths=[col_w] * max(len(r) for r in rows))
                tbl.setStyle(TableStyle([
                    ('BACKGROUND', (0, 0), (-1, 0), INK),
                    ('TEXTCOLOR', (0, 0), (-1, 0), HEX_WHITE),
                    ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
                    ('FONTSIZE', (0, 0), (-1, 0), 9),
            ...
            for code_line in block['text'].split('\n'):
                story.append(Paragraph(code_line.replace(' ', '\xa0'), styles['CNCode']))
        elif t == 'p':
            parts = re.split(r'(\*\*.*?\*\*)', block['text'])
            para_parts = []
            for part in parts:
...
class ExportService:
    ...
    def export(self, markdown_content: str, format: ExportFormat) -> BytesIO:
        """
        ...
        if format not in self.SUPPORTED_FORMATS:
            raise ValueError(f"不支持的导出格式: {format}，可选: {self.SUPPORTED_FORMATS}")

        blocks = _parse_md_blocks(markdown_content)

        if format == "docx":
            return _build_docx(blocks)
        else:  # pdf
            return _build_pdf(blocks)
    ...
    def get_filename(self, format: ExportFormat, base_name: str = "notes") -> str:
        """获取导出文件名。"""
